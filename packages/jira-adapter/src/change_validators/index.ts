...
export default (client: JiraClient, config: JiraConfig, paginator: clientUtils.Paginator): ChangeValidator => {
  const validators: Record<ChangeValidatorName, ChangeValidator> = {
    ...
    issueTypeSchemeMigration: issueTypeSchemeMigrationValidator(client, config),
    activeSchemeChange: activeSchemeChangeValidator(client, config),
    masking: maskingValidator(client),
    issueTypeDeletion: issueTypeDeletionValidator(client, config),
