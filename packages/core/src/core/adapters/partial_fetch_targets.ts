...
export const getAccountPartialFetchTargets = ({
  account,
  workspace,
  adapterCreators,
}: {
  ...
  log.timeDebug(
    async () => {
      ...
      return adapterCreators[adapter]?.partialFetch?.getAllTargets({
    ...
    'getAccountPartialFetchTargets for account %s',
    account,
  )

const getTargetsForAccountElements = async ({
  adapter,
  account,
  accountElemIds,
  elementsSource,
...
export const getPartialFetchTargetsForElements = ({
  ...
  log.timeDebug(
    async () => {
      ...
      return mapValuesAsync(elemIdsByAccount, (accountElemIds, account) =>
    ...
    'getPartialFetchTargetsForElements for %d ids',
    elemIds.length,
  )
